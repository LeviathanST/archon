...
> Master Plan v1.0 | Created: 2026-03-09
> Status: Planning
...
│   │   ├── relevance.ts             # Relevance detector implementation
...
- [ ] Clean workspace (remove Zig scaffold)
- [ ] Initialize: `package.json`, `tsconfig.json`, `docker-compose.yml` (Postgres 16)
- [ ] `src/db/schema.ts` — Drizzle schema for all tables
- [ ] `src/db/connection.ts` — Postgres pool
- [ ] Generate and run initial migration
- [ ] `src/protocol/messages.ts` — Zod schemas for auth messages
- [ ] `src/hub/server.ts` — WebSocket server with `ws`
- [ ] `src/hub/session.ts` — session tracking
- [ ] `src/hub/router.ts` — message dispatch (auth only)
- [ ] `src/utils/logger.ts` — pino setup
- [ ] `src/index.ts` — boot sequence
- [ ] Manual test: connect with `wscat`, auth, receive auth.ok
...
- [ ] Create CEO agent workspace template in `agents/ceo/`
- [ ] `src/db/seed.ts` — CEO agent, sample departments (engineering, research, planning), roles
- [ ] `src/registry/agent-card.ts` — read SOUL.md + IDENTITY.md, merge with DB, produce AgentCard
- [ ] `src/registry/agent-registry.ts` — CRUD operations
- [ ] `src/registry/discovery.ts` — permission-filtered agent listing
- [ ] `src/hub/permissions.ts` — permission checking
- [ ] Add `directory.list` and `directory.get` to router
- [ ] Tests for registry and discovery
...
- [ ] `src/meeting/phases.ts` — phase state machine with transition rules
- [ ] `src/meeting/meeting-room.ts` — MeetingRoom class (create, join, broadcast, store messages)
- [ ] `src/meeting/turn-manager.ts` — relevance collection, timeout (10s), turn ordering
- [ ] `src/utils/token-counter.ts` — tiktoken wrapper
- [ ] Add all `meeting.*` handlers to router
- [ ] Budget enforcement: per-phase tracking, auto-advance on exhaustion
- [ ] Meeting persistence: save decisions + action items to Postgres
- [ ] Tests for phase transitions, budget exhaustion, turn management
...
- [ ] `src/meeting/relevance.ts` — relevance prompt builder
- [ ] Agent client library (TypeScript) — wraps WebSocket connection:
  - Auth handshake
  - Meeting participation
  - Relevance detector (runs prompt via acpx/api/cli)
  - Reconnect logic
- [ ] Agent runner script — reads SOUL.md/IDENTITY.md, connects, participates in meetings
- [ ] Neural Memory integration — agent consults brain during relevance check and before speaking
- [ ] Integration test: hub + 3 agents, create meeting, observe emergent discussion
...
- [ ] CEO capabilities: create agents, departments, assign roles, manage templates
- [ ] Agent creation flow: CEO → template → workspace → DB registration → ready
- [ ] Project CRUD with methodology selection (waterfall/scrum/kanban)
- [ ] Link meetings to projects
- [ ] DECIDE phase: proposal + voting mechanics
- [ ] ASSIGN phase: task creation + acknowledgement
...
- [ ] **Web UI / Dashboard** — visual org chart, meeting viewer, analytics
